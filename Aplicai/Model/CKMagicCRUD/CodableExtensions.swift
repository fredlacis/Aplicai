...
extension Decodable {
    ...
    static func load(from dictionary:[String:Any])throws ->Self{
        do {
            guard let data:Data = (dictionary[String(describing: self)] as? [Any])?.asData ??
                                   dictionary.asData else {
                    debugPrint("1 - Can not convert from dictionary to", String(describing: type(of: self)))
                    throw FileManageError.canNotConvertData
            }
            return try Self.load(from: data)
        } catch {
            debugPrint("2 - Can not convert from dictionary to", String(describing: type(of: self)))
            throw FileManageError.canNotConvertData
        }
    }
    ...
    static func load(from array:[Any])throws ->Self{
        ...
        do {
            guard let data = array.asData else {
                debugPrint("3 - Can not convert from array to", String(describing: type(of: self)))
                throw FileManageError.canNotConvertData
            }
            return try Self.load(from: data)
        } catch {
            debugPrint("4 - Can not convert from array to", String(describing: type(of: self)))
            throw FileManageError.canNotConvertData
        }
    }
